...
class Link(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    movie_id = db.Column(db.Integer, db.ForeignKey('movies.id'), nullable=False)
    imdb_id = db.Column(db.String(10), nullable=True)  # Adjust the length as needed
    tmdb_id = db.Column(db.Integer, nullable=True)

    # Relationships
    movie = db.relationship('Movie', backref=db.backref('link', lazy=True))
...
class Tag(db.Model):
    ...
    timestamp = db.Column(db.DateTime, nullable=False)  # Assuming timestamp is a date/time field

    # Relationships
    user = db.relationship('User', backref=db.backref('tags', lazy=True))
    movie = db.relationship('Movie', backref=db.backref('tags', lazy=True))